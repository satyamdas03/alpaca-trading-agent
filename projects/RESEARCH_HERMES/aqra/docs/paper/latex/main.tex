...
\node[box, below left=7mm and 1mm of ledger] (train) {train data\\$T$};
\node[box, below right=7mm and 1mm of ledger] (val) {validation\\$V$};
...
\draw[arr] (val) -- (fdr);
\draw[arr] (fdr) -- (verif);
\draw[dashed, thick] ($(train.north east)!0.5!(val.north west) + (0,2mm)$)
  -- ($(train.south east)!0.5!(val.south west) - (0,2mm)$)
  node[midway, right=1mm, font=\tiny] {wall};
\end{tikzpicture}
